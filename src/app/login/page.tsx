  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (profileSnap.exists()) {
        toast({
          title: "Welcome Back!",
          description: `Signed in as @${profileSnap.data().username}`,
        });
        router.push("/");
      } else {
        toast({
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen flex flex-col justify-center p-6 space-y-8 bg-background">
      <div className="text-center space-y-2">
        <h1 className="text-4xl font-black text-primary uppercase tracking-tighter">InnovateSphere</h1>
        <p className="text-sm text-muted-foreground font-medium uppercase tracking-widest">Orange Edition</p>
      </div>
