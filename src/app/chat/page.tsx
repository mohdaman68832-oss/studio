...
import { cn } from "@/lib/utils"; // Fixed Build Error: Import path updated to @/lib/utils
import Link from "next/link";

export default function ChatPage() {
  const { user, loading: isUserLoading } = useUser(); // Fixed ReferenceError: user loading state handled
  const db = useFirestore();
  const [searchQuery, setSearchQuery] = useState("");

  const messagesQuery = useMemoFirebase(() => {
    if (!db || !user) return null;
    // Privacy Logic: Show messages where the user is either sender or receiver
    return query(
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-background pt-8 pb-24">
      <div className="px-6 mb-8">
        ...
        <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest">Secure Connections</p>
      </div>
      ...
      <div className="px-6 space-y-4">
        {isMessagesLoading ? (
          ...
          <div className="py-24 text-center space-y-4 opacity-30 px-10">
            ...
            <p className="text-[9px] font-medium italic">Start a conversation to see it here!</p>
          </div>
        )}
      </div>
    </div>
