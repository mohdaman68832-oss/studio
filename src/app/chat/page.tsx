  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-background pt-8 pb-24">
      <div className="px-6 mb-8">
        ...
        <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest">Safe Conversations</p>
      </div>
      ...
      <div className="px-6 space-y-4">
        {isMessagesLoading ? (
          ...
          messages.map((msg) => (
            <Link key={msg.id} href={`/chat/${msg.senderId}`} className="flex items-center gap-4 bg-card p-4 rounded-[2rem] border border-border/50 shadow-sm hover:border-primary transition-all">
               <Avatar className="h-12 w-12 border-2 border-background">
                 <AvatarFallback className="font-black bg-primary/10 text-primary">U</AvatarFallback>
               </Avatar>
               <div className="flex-1 min-w-0">
                  <h4 className="text-sm font-black truncate">Innovator</h4>
          ...
          <div className="py-24 text-center space-y-4 opacity-30 px-10">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.2em]">Inbox Clear</p>
            <p className="text-[9px] font-medium italic">Start an innovation collaboration to chat.</p>
          </div>
        )}
      </div>
    </div>
